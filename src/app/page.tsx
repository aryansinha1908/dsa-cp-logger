...
function StatsCard() {
      ...
      .then((d) => {
        ...
        for (let i = 0; ; i++) {
          ...
          if (days.includes(d2.toLocaleDateString("en-CA"))) current++;
            ...
            break; // today empty — check yesterday
          else if (i === 1 && !days.includes(d2.toLocaleDateString("en-CA")))
            break;
          else break;
        }
        ...
        if (!days.includes(todayStr)) {
          ...
          for (let i = 0; ; i++) {
            const d2 = new Date(yest);
            d2.setDate(yest.getDate() - i);
            if (days.includes(d2.toLocaleDateString("en-CA"))) current++;
            else break;
          }
        }
        ...
        const thisWeek = d.filter(
          (p: any) => new Date(p.createdAt) >= monday,
        ).length;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-5 mb-8">
        ...
        <div className="min-w-0">
          <p className="text-2xl font-semibold text-foreground truncate">
            {session?.user?.name ?? "Your archive"}
          </p>
          <p className="text-base text-muted-foreground truncate">
            {session?.user?.email ?? "Sign in to see stats"}
          </p>
        </div>
      </div>
      ...
      {/* Total */}
      <div className="mb-8">
        <p className="text-8xl font-bold text-foreground tabular-nums">
          {stats?.total ?? "—"}
        </p>
          ...
          Object.entries(stats.byPlatform)
            ...
            .map(([platform, count]) => (
              <div key={platform} className="flex items-center gap-4">
                ...
                <span className="text-base text-muted-foreground w-32 truncate text-right">
                  {platform}
                </span>
                <span className="text-base font-medium text-foreground w-8 text-right tabular-nums">
                  {count}
                </span>
              </div>
            ))}
        ...
        <div className="flex items-center gap-3">
          {isNewRecord ? (
            <span
              ...
              style={{
                filter:
                  "drop-shadow(0 0 10px #f97316) drop-shadow(0 0 20px #f97316)",
              }}
            >
              🔥
            </span>
            ...
            <div className="relative w-16 h-16 shrink-0">
              <svg className="w-16 h-16 -rotate-90" viewBox="0 0 72 72">
                ...
                <circle
                  ...
                  strokeDashoffset={circ * (1 - streakPct)}
                  strokeLinecap="round"
                />
              </svg>
    ...
    >
      ...
      <section className="py-28 md:py-36 border-b border-border">
        <div className="flex items-center justify-between gap-12">
          ...
          >
            <motion.p
              variants={fadeUp}
              className="text-sm font-mono text-primary uppercase tracking-widest mb-8"
            >
              DSA / CP Logger
            </motion.p>
            <motion.h1
              variants={fadeUp}
              className="text-5xl md:text-7xl font-bold tracking-tight text-foreground leading-tight mb-8"
            >
              Build your personal
              <br className="hidden md:block" /> problem archive.
            </motion.h1>
            <motion.p
              variants={fadeUp}
              className="text-xl text-muted-foreground max-w-xl mb-12 leading-relaxed"
            >
              Log every DSA and competitive programming problem you solve.
              Auto-fetch metadata, track insights and mistakes, review before
              interviews.
            </motion.p>
            <motion.div
              variants={fadeUp}
            ...
            >
              <Button
                ...
                asChild
                style={{ anchorName: "--tour-btn-new" } as any}
              >
                <Link href="/problems/new">
                  Log a problem <ArrowRight className="ml-2 w-4 h-4" />
                </Link>
              </Button>
              <Button
                size="lg"
                ...
                className="h-12 px-8 text-base"
                asChild
                style={{ anchorName: "--tour-btn-view" } as any}
              >
      ...
      <section className="py-24 border-b border-border">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground">
            Three steps to build your archive.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              n: "01",
              title: "Solve & copy the URL",
              desc: "Finish a problem on LeetCode, Codeforces, or any platform and copy the problem URL.",
            },
            {
              n: "02",
              title: "Paste & auto-fetch",
              desc: "Paste the link — we pull the title, difficulty, and topic tags from the platform automatically.",
            },
            {
              n: "03",
              title: "Add notes & review",
              desc: "Write your key insight and any mistakes. Come back to review the whole log before interviews.",
            },
      ...
      <section className="py-24 border-b border-border">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground">
            Auto-fetch works on LeetCode and Codeforces. Others can be logged
            manually.
          </p>
        </motion.div>
      ...
      <section className="py-32">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-md leading-relaxed">
            It's free. Takes 10 seconds to log a problem. Start building the
            habit now.
          </p>
          <Button size="lg" asChild>
            <Link href="/problems/new">
              Log your first problem <ArrowRight className="ml-2 w-4 h-4" />
            </Link>
          </Button>
        </motion.div>
      </section>
      ...
      <TourStep
        step={1}
        title="Log a problem"
        content="Paste a URL and we fetch everything automatically. Add your insights and you're done."
      ...
      <TourStep
        step={2}
        title="View your log"
        content="See all your solved problems. Filter by difficulty, platform, tags, or how you solved it."
      ...
      <TourStep
        step={3}
        title="Core features"
        content="Auto-fetch, smart filters, and insight tracking — the full loop from solve to review."
