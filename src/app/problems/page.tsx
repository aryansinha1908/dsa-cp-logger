          ...
          <div className="max-h-52 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              ...
              filtered.map((tag) => (
                ...
                >
                  <div
                    className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border ${selectedTags.includes(tag) ? "bg-primary border-primary" : "border-input"}`}
                  >
                    {selectedTags.includes(tag) && (
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-3 pb-4 border-b border-border">
        ...
        <Select
          value={solvedBy}
          onValueChange={(v) => setSolvedBy(!v || v === "All" ? "" : v)}
        >
          <SelectTrigger
            className="w-40"
            style={{ fontFamily: "var(--font-atkinson-mono), monospace" }}
      ...
      {loading ? (
        ...
        <div className="py-24 text-center border border-dashed border-border rounded-lg space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Try adjusting your filters or log a new problem.
          </p>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {paginated.map((p: any) => (
              <div
                key={p.id}
                className="group relative flex flex-col gap-3 rounded-xl border border-border bg-card p-4 hover:border-primary/40 transition-colors"
              >
                {/* Title row */}
                <div className="flex items-start justify-between gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="font-medium text-foreground hover:text-primary transition-colors leading-snug line-clamp-2 flex-1"
                  >
                    {p.title}
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="shrink-0 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-primary"
                  >
                ...
                <div className="flex items-center gap-2 flex-wrap text-xs text-muted-foreground">
                  ...
                  {p.difficulty && (
                    <span
                      className={`px-1.5 py-0.5 rounded font-medium ${DIFFICULTY_STYLE[p.difficulty] ?? "text-muted-foreground bg-muted"}`}
                    >
                      {p.difficulty}
                    </span>
                  )}
